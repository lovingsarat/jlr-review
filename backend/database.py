...
def get_markdown_summary() -> str:
    ...
    for item in FEEDBACK_ITEMS:
        upcoming_tag = " (Upcoming Planned Activity)" if item["isUpcoming"] else ""
        text_escaped = item["text"].replace("|", "\\|")
        lines.append(
            f"| {item['platform']} | {item['author']} | {item['date']} | {item['event']}{upcoming_tag} | {item['sentiment']} | {item['city']} | {text_escaped} |"
        )
    return "\n".join(lines)
